...
class VideoGetter:
    ...
    def start(self):
        if self.started:
            ...
            self.started = True
            self.thread = threading.Thread(target=self.runTime, name='VideoGetFrameThread', daemon=True)
            self.thread.start()
...
class VideoShower:
    ...
    def start(self):
        ...
        if self.started:
            ...
            self.started = True
            self.thread = threading.Thread(target=self.runTime, name='VideoShowThread', daemon=True)
            self.thread.start()
...
if __name__ == "__main__":
    ...
    startTime = time.time()
    while True:
        if isinstance(vget.bucket, np.ndarray):
            vshow.frame = vget.bucket
            vget.bucket = None
